...
import { useCheckoutCartMutation, useDeleteCartMutation, useGetMyCartsQuery, useUpdateCartQuantityMutation } from "../../redux/api/cartAPI";
...
const UserCart = () => {
    ...
    const [updateCartQuantity, { isLoading: updateQuantityLoading, isSuccess: isSuccessQuantity, error: errorQuantity, isError: isErrorQuantity, data: dataQuantity }] = useUpdateCartQuantityMutation();
    ...
    const handleQuantityChange = async (cartId, newQuantity) => {
        ...
        await updateCartQuantity({ id: cartId, quantity: newQuantity });
    };
    ...
    useEffect(() => {
        if (isSuccessQuantity) {
            // toast.success(dataQuantity?.message);
            // refetchMyCart();
        }
        if (isErrorQuantity) {
            if (isSuccessQuantity && dataQuantity?.message) {
                toast.success(dataQuantity.message);
            } else if (isErrorQuantity && errorQuantity?.data) {
                const errorMessage = typeof errorQuantity.data === 'string' ? errorQuantity.data : errorQuantity.data?.message;
                toast.error(errorMessage || 'An unknown error occurred', {
                    position: 'top-right',
                });
            }
                    ...
                    <Container>
                        ...
                        <>
                            ...
                            <div className="d-flex mt-3 justify-content-between">
                                <Link to="/dashboard" className="btn btn-outline-primary">Go shopping</Link>
                                {cartItems.length > 0 && (
                                    <button type="button" className="btn btn-outline-success" onClick={handleCheckout}>Check out</button>
                                )}
                            </div>
                        </>
                    </Container>
