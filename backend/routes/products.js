...
router.get('/', verifyToken(['admin', 'user']), async (req, res) => {
    try {
        // Build resolution filter if provided
        const resolutionFilter = req.query.resolutions ?
            { resolution: { $in: req.query.resolutions.split(',') } } : {};

        const sizeFilter = req.query.sizes ?
            { size: { $in: req.query.sizes.split(',') } } : {};

        // Build search query filter
        const searchQuery = req.query.searchItem || '';
        const searchFilter = {
            $or: [
...
router.post('/search', async (req, res) => {
    try {
        ...
        const resolutionFilter = Array.isArray(req.body.resolutions) && req.body.resolutions.length > 0 ?
            { resolution: { $in: req.body.resolutions } } : {};

        const sizeFilter = Array.isArray(req.body.sizes) && req.body.sizes.length > 0 ?
