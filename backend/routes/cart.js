...
router.put('/update/:cartId', verifyToken(['user']), async (req, res) => {
    ...
    try {
        ...
        if (!cartItem) {
            return res.status(404).send({ status: "error", message: "Cart item not found" });
        }

        let productData = await Product.findById(new ObjectId(cartItem.product));
        if (!productData) {
            return res.status(404).send({ status: "error", message: "Product not found" });
        }

        let newQuantity = parseFloat(quantity);
        let oldQuantity = parseFloat(cartItem.quantity);
        let stockChange = oldQuantity - newQuantity; // Positive value means stock increase, negative means stock decrease

        let changedStock = parseFloat(productData.stock) + stockChange;
        if (changedStock < 0) {
            return res.status(400).send({ status: "error", message: "Insufficient stock" });
        }

        cartItem.quantity = newQuantity;
        await cartItem.save();

        await Product.findOneAndUpdate({ _id: new ObjectId(cartItem.product) }, { stock: changedStock });
        
        return res.status(200).send({ status: "success", message: "Cart updated successfully!" });
    } catch (error) {
        return res.status(500).send({ status: "error", message: error.message });
    }
});
